...
window.addEventListener("load",_=>{
  ...
  function makeRender(vss,fss,locs){
    ...
    function setting(spec, name, args){
      let n = args.length;
      let func = "uniform" + n + spec;
      let as = [];
    ...
    let setter = {i: {}, f: {}};
    locs.forEach(loc=>{
      setter.i[loc] = function(){setting("iv",loc,arguments);};
      setter.f[loc] = function(){setting("fv",loc,arguments);};
    });
    return f=>{
  ...
  R = {
    ...
    text:(x,y,s,align,text,c)=>{
      ctx.textAlign = align;
      ctx.fillStyle = "rgb("+c+","+c+","+c+")";
      ctx.font = (s/2) + "px 'number'";
      ctx.fillText(text,x,scrH-y);
    }
  };
