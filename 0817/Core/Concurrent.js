Util.register("Concurrent","Q",_=>{
  ...
  c.console = box=>c.do(function*(){
    while(1)Log.def(yield box.receive);
  });
  ...
  c.listen = proc=>Q.fork(Q.do(function*(){
    while(1){
      yield proc;
    }
  }));
  ...
  c.sequence = proc=>c.do(function*(){
    ...
    while(cur<conds.length){
      yield c.join.any(conds.slice(cur));
      yield execs[cur](result);
      cur++;
    }
  });

  // C () -> C ()
  c.synchronized = proc=>c.do(function*(){
    const lock = c.newBox({});
    const h = yield getHandler;
    const handle = {
      check : _=>true,
    ...
    yield putHandler(merge(h,handle));
    yield proc;
    yield putHandler(h);
  });
  // C () -> C ()
  c.atomic = act=>c.do(function*(){
    const h = yield getHandler;
    if(h.blocks.length==0)throw new Error("No synchronized threads");
    const lock = h.blocks[h.blocks.length-1];
    yield takeBox(lock);
    const x = yield act;
    yield putBox(lock,{});
    return c.pure(x);
