...
var Shape = (()=>{
  ...
  s.leaf = (z)=>s.make((ctx)=>{
    ctx.moveTo(0,0);
    ctx.bezierCurveTo(z/3,0,z/2,-z/2,0,-z);
    ctx.bezierCurveTo(-z/2,-z/2,-z/3,0,0,0);
  },(x,y)=>false);
  s.image = (name)=>{
  ...
  s.imageClipped = (name,x1,y1,w1,h1)=>{
    var img = Picture.get(name);
    if(!img)return;
    return s.make((ctx)=>{
  ...
  var font;
  opentype.load('/Jam/0611/font.ttf',(err,f)=>{
